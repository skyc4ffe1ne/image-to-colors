...
export default function Canvas({ setPalette, setCustomPalette }: CanvasProps) {
	...
	let currImageRef = useRef<null | HTMLImageElement>(null);
	...
	useEffect(() => {
		...
		const responsiveCanvas = () => {
			...
			canvas.width = Math.min(700, width);
			if(currImageRef && currImageRef.current){
				ctx.drawImage(currImageRef.current, 0, 0, canvas.width, canvas.height);
			}
		};
		...
		return () => window.removeEventListener('resize', responsiveCanvas);
	}, []);
		...
		<div className="">
			...
			>
				...
				>
					...
					{!picture && (
						<PlaceholderImage className="absolute lg:top-1/2 left-1/2 lg:-translate-x-3/4 lg:-translate-y-1/2 top-0 -translate-x-1/2 w-full max-w-fit rounded-xl" />
					)}
